  ...
  async execute(interaction) {
    ...
    Icons.forEach((icon , i) =>{
        if(i <= 12){
            ...
            commandsEmbed1.addFields({name:`Icon ${i+1}`, value: `${icon}`, inline:true})
        }if(i <= 25 &&  i > 12){

            commandsEmbed2.addFields({name:`Icon ${i+1}`, value: `${icon}`, inline:true})

        }
        if(i <= 38 && i > 12 && i > 25){
            commandsEmbed3.addFields({name:`Icon ${i+1}`, value: `${icon}`, inline:true})

        }
    })
    await interaction.reply({ embeds: [commandsEmbed1, commandsEmbed2, commandsEmbed3], ephemeral:true });
    }
  },
};
